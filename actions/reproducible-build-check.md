...
The simplest form — runs on every push to the listed branches:
...
#### Scheduled caller (multiple branches)

A scheduler can only run on the default branch. Use the `branch` input to tell
the reusable workflow which branch to check out and resolve configuration for:
...
jobs:
  check:
    strategy:
      fail-fast: false
      matrix:
        branch: [ main, '1.x', '2.x' ]   # tune to your branches
    uses: hibernate/.github/.github/workflows/reproducible-build-check.yml@main
    with:
...
The `branch` input is optional — when omitted (e.g. on a `push` trigger), the
workflow defaults to the triggering ref.
